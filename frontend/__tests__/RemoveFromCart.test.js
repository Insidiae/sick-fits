...
// Just redirect whatever alert() displays to console.log()
global.alert = console.log;

const mocks = [
  // CURRENT_USER_QUERY mock
  // Since the current user's data is updated via an update() method,
  // we only need to mock the response once here
  {
  ...
  // REMOVE_FROM_CART_MUTATION mock
  {
    request: {
...
describe('<RemoveFromCart/>', () => {
  ...
  it('removes the item from cart', async () => {
    ...
    // Wait until the Apollo Client gets updated
    await wait();
    const { data: { me: updatedMe } } = await apolloClient.query({ query: CURRENT_USER_QUERY });
    expect(updatedMe.cart).toHaveLength(0);
  });
});